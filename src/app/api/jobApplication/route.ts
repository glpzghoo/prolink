...
export async function GET(req: NextRequest) {
  try {
    ...
    if (verify.role === 'CLIENT') {
      ...
      return CustomNextResponse(true, 'REQUEST_SUCCESS', 'Компаний анкетүүд амжилттай татлаа!', {
        user,
      });
      ...
      return CustomNextResponse(true, 'REQUEST_SUCCESS', 'Талентын анкетүүд амжилттай татлаа!', {
        user,
      });
    } else {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!id) {
      return CustomNextResponse(false, 'NO_ID_PROVIDED', 'Хүсэлт амжилтгүй! ID олдсонгүй!', null);
    }
    if (!process.env.ACCESS_TOKEN) {
      return NextResponse_NoEnv('ACCESS_TOKEN');
    }
    ...
    return CustomNextResponse(true, 'REQUEST_SUCCESS', 'Амжилттай устгалаа!', jobApplication);
  } catch (err) {
    console.error(err, 'Сервер дээр асуудал гарлаа!');
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (jobApplication.clientStatus === 'accepted' && statusValue === 'accepted') {
      return CustomNextResponse(false, 'REQUEST_FAILED', 'Юу ч өөрчлөгдөөгүй!', jobApplication);
    }
    ...
    if (statusValue === 'accepted') {
      ...
      await transporter.sendMail({
        ...
        <h2>Талентын тухай дэлгэрэнгүй мэдээлэл!</h2>
            ...
            <br> 
             <br>
            <strong>Нас</strong>: ${
              new Date().getFullYear() - new Date(jobApplication.freelancer.birthday).getFullYear()
            }
             <br>
              <br>
             <strong>Утасны дугаар: </strong> ${jobApplication.freelancer.phoneNumber}
            <br> 
             <br>
             <strong>Манайд гишүүн болоод: </strong> ${
               new Date(jobApplication.freelancer.createdAt).toISOString().split('T')[0]
             }
            <br> 
             <br>
        ...
        html: `<h1><b>Сайн байна уу! - ${
          jobApplication.client.companyName
           ...
           ${jobApplication.client.firstName}
            <br> 
             <br>
             <strong>Утасны дугаар: </strong> ${jobApplication.client.phoneNumber}
            <br> 
             <br>
             <strong>Манайд гишүүн болоод: </strong> ${
               new Date(jobApplication.client.createdAt).toISOString().split('T')[0]
             }
            <br> 
             <br>
            <strong>Нийт анкетны тоо: </strong> ${jobApplication.client.clientJobApplication.length}
            <br> 
             <br>
            <strong>Манай сайт дээрх дундаж үнэлгээ: </strong> ${avgRating(
      ...
      return CustomNextResponse(true, 'REQUEST_SUCCES', 'Төлөв амжилттай өөрчлөгдлөө.', {
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    return CustomNextResponse(false, 'REQUEST FAILED', 'Төлөв өөрчилж чадсангүй.', null);
  } catch (err) {
    console.error(err, 'Сервер дээр асуудал гарлаа!');
    return NextResponse_CatchError(err);
  }
