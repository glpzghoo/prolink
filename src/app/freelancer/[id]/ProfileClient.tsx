...
export default function Client() {
  ...
  const handleLeftScroll = () => div.current?.scrollBy({ left: -400, behavior: 'smooth' });
  const handleRightScroll = () => div.current?.scrollBy({ left: 400, behavior: 'smooth' });
    ...
    const fetchData = async () => {
      try {
        ...
        if (res1.data.success) {
          ...
          const filter = uniqs.filter((s) => s.id !== id && s.role === 'FREELANCER');
  ...
  const copyURL = () => {
    navigator.clipboard.writeText(window.location.href).then(() => setAlert(true));
  };
      ...
      {user.companyName ? (
        ...
        >
          ...
          <div className="mt-8 border-t pt-6">
            ...
            <div ref={div} className="flex gap-4 overflow-x-auto scrollbar-hide pb-4">
          ...
          <div className="mt-8 border-t pt-6">
            ...
            <Textarea
              ...
              onChange={(e) => setRatingForm({ ...ratingForm, message: e.target.value })}
